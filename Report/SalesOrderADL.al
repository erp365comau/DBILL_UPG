report 50001 "Sales Order - AD - L"
{
    ...
    // </changelog>
    // 
    // J2716 15Mar2013 LK - design layout for given format
    DefaultLayout = RDLC;
    RDLCLayout = 'Report/SalesOrderADL.rdl';
    ApplicationArea = all;
    Caption = 'Sales Order - AD';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(SalesHeaderSellToContact; "Sales Header"."Sell-to Contact")
            {
            }
            column(SalesHeaderAssignedUserId; "Sales Header"."Assigned User ID")
            {
            }
            column(SalesHeaderPostingDate; "Sales Header"."Posting Date")
            {
            }
            column(ComentLine1; CommentLine[1])
            {
            }
            column(ComentLine2; CommentLine[2])
            {
            }
            column(ComentLine3; CommentLine[3])
            {
            }
            column(ComentLine4; CommentLine[4])
            {
            }
            column(ComentLine5; CommentLine[5])
            {
            }
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(ShipToAddr5_1; ShipToAddr[5])
                    {
                    }
                    column(ShipToAddr4_1; ShipToAddr[4])
                    {
                    }
                    column(ShipToAddr3_1; ShipToAddr[3])
                    {
                    }
                    column(ShipToAddr2_1; ShipToAddr[2])
                    {
                    }
                    column(ShipToAddr1_1; ShipToAddr[1])
                    {
                    }
            ...
            begin
                ...
                // >> J2716
                SalesCommentLine.SETRANGE(SalesCommentLine."Document Type", "Document Type");
                SalesCommentLine.SETRANGE(SalesCommentLine."No.", "No.");
                i := 0;
                IF SalesCommentLine.FINDSET THEN
                    REPEAT
                        i := i + 1;
                        CommentLine[i] := SalesCommentLine.Comment;
                    UNTIL (SalesCommentLine.NEXT = 0) OR (i = 5);
                // << J2716
            end;
    ...
    var
        ...
        OrderNoCaptionLbl: Label 'Number';
        ...
        CommentLine: array[6] of Text[100];
