report 50016 Checks
{
    ...
    // Austral Sugeevan 06/10/2014 >>> Fixed Amount Format Issue
    // Austral Sugeevan 09/10/2014 >>> RTC Layout Mods on Adding "DOLLARS" and $ in the Check Amount and Check Amount Text
    // Austral Sugeevan 05/03/2015 >>> Print Gen Journal Line Description in the footer instead of printing Name of Customer, Vendor or Bank Account
    // Austral Sugeevan 18/11/2015 >>> RTC Layout Mods
    // Austral Sugeevan 19/11/2015 >>> Changed Cheque part Date Format
    DefaultLayout = RDLC;
    RDLCLayout = 'Report/Check.rdl';
    ApplicationArea = all;
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            dataitem(CheckPages; Integer)
            {
                ...
                dataitem(PrintCheck; Integer)
                {
                    ...
                    column(CheckToAddr1_PrintCheck; GenJnlLineDescription)
                    {
                    }
                                    ...
                                    IF STRLEN(FORMAT(Decimals)) < STRLEN(FORMAT(Currency."Amount Rounding Precision")) THEN
                                        IF Decimals = 0 THEN
                                            CheckAmountText := FORMAT(CheckLedgEntry.Amount, 0, 0)// +
                                                                                                  ...
                                                                                                  //PADSTR('',STRLEN(FORMAT(Currency."Amount Rounding Precision"))-2,'0')//Austral Sugeevan 06/10/2014
                                        ELSE
                                            CheckAmountText := FORMAT(CheckLedgEntry.Amount, 0, 0)// +
                                                                                                  //PADSTR('',STRLEN(FORMAT(Currency."Amount Rounding Precision"))-STRLEN(FORMAT(Decimals)),'0')//Austral Sugeevan 06/10/2014
                                    ELSE
                                        CheckAmountText := FORMAT(CheckLedgEntry.Amount, 0, 0);
            ...
            begin
                ...
                CLEAR(GenJnlLineDescription); //Austral Sugeevan 05/03/2015 <<<
                IF NOT TestPrint THEN BEGIN
                    IF Amount = 0 THEN
                        CurrReport.SKIP;
                    ...
                    //Austral Sugeevan 19/11/2015 <<<
                    GenJnlLineDescription := Description; //Austral Sugeevan 05/03/2015 <<<
                END ELSE BEGIN
                    ...
                    GenJnlLineDescription := Text003; //Austral Sugeevan 05/03/2015 <<<
                END;
            end;
